        ...
        while (itr.hasNext())//checking if there is any value in the position like rs.next()
        {
            ...
            } else//when it is an image or multipart
            {
                ...
                if (field_name.equals("img_photo"))//write the name of image here it is img_product
                {
                    ...
                    String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Files\\AddON\\";//path for storing image
                    Random r = new Random();//to generate random number
                    int r_num = r.nextInt(1111) + 999;//values between this are generated

                    photo = "AddOnPhoto_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
...
//                        }
    ...
    String InsQry = "insert into tbl_add_on(course_id,add_on_name,add_on_title,add_on_photo)"
            + "values('" + value[0] + "','" + value[1] + "','" + value[2] + "','" + photo + "')";
...
<script type="text/javascript">
    ...
    setTimeout(function() {
        window.location.href = '../../Department/AddOn.jsp'
